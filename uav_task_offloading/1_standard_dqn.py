...
"""
================================================================================
STANDARD DQN FOR UAV TASK OFFLOADING
================================================================================
Per-Device Binary Decision Architecture -- Scales to any number of devices.
Standard Double DQN with soft target updates for binary task offloading
in UAV-assisted edge computing networks.

To run: python 1_standard_dqn.py
...
def train():
    ...
    print(f"\n{'=' * 70}")
    print(f"  STANDARD DQN -- {N_DEVICES} Devices, 1 UAV, {N_EPISODES} Episodes")
...
if __name__ == "__main__":
    ...
    # Summary
    print(f"\n{'=' * 50}")
    print(f"  FINAL RESULTS -- Standard DQN")
    print(f"{'=' * 50}")
    results = [
        ("Standard DQN", np.mean(lat[-100:])),
    ...
    fig.suptitle(f"Standard DQN -- UAV Task Offloading\n"
                 f"{N_DEVICES} Devices, {N_EPISODES} Episodes",
                 fontsize=14, fontweight='bold')

    # Reward
    axes[0, 0].plot(rew, alpha=0.15, color='purple')
    axes[0, 0].plot(x_range, moving_avg(rew, window),
                    color='purple', linewidth=2.5, label='Standard DQN')
    ...
    # Latency
    axes[0, 1].plot(lat, alpha=0.15, color='purple')
    ...
    # Loss
    axes[1, 0].plot(losses, alpha=0.15, color='purple')
    ...
    bar_colors = ['purple', 'darkorange', 'green', 'red']
    ...
    plt.tight_layout()
    plt.savefig('outputs/1_standard_dqn.png', dpi=150, bbox_inches='tight')
    plt.show()
    print("\nPlot saved as outputs/1_standard_dqn.png")
