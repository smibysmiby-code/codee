...
"""
================================================================================
A2C (ADVANTAGE ACTOR-CRITIC) BASELINE FOR UAV TASK OFFLOADING
================================================================================
Per-Device Binary Decision Architecture -- Scales to any number of devices.
Synchronous Advantage Actor-Critic with entropy regularization and n-step
returns for binary task offloading in UAV-assisted edge computing.

To run: python 5_a2c_baseline.py
...
# =============================================================
#  A2C ACTOR-CRITIC NETWORK (Per-Device Binary Decisions)
# =============================================================
class A2CPerDeviceNet(nn.Module):
    """
    ...
    Separate actor/critic tails for reduced interference.
    """
...
class A2CAgent:
    def __init__(self, state_size):
        ...
        self.model      = A2CPerDeviceNet(state_size).to(DEVICE)
    ...
    def act(self, state):
        ...
        log_prob = dists.log_prob(actions).sum()
        entropy  = dists.entropy().sum()

        self.states.append(state)
        self.actions.append(actions.cpu().tolist())
    ...
    def update(self, next_state, done):
        if len(self.rewards) < N_STEPS and not done:
            return None

        with torch.no_grad():
            state_t = torch.FloatTensor(next_state).unsqueeze(0).to(DEVICE)
            _, bootstrap_val = self.model(state_t)
        R = 0.0 if done else bootstrap_val.item()
...
def train():
    ...
    env   = UAVEnvironment()
    agent = A2CAgent(env.state_size)

    latencies, drops, rewards, losses = [], [], [], []

    print(f"\n{'=' * 70}")
    print(f"  A2C BASELINE -- {N_DEVICES} Devices, 1 UAV, {N_EPISODES} Episodes")
    print(f"  Per-device binary decisions (Actor-Critic)")
    print(f"  n-step={N_STEPS}, entropy_coeff={ENTROPY_COEFF}, "
          f"gamma={GAMMA}")
    print(f"{'=' * 70}")
    ...
    for ep in range(N_EPISODES):
        ...
        for slot in range(N_SLOTS):
            ...
            if (slot + 1) % N_STEPS == 0 or done:
                loss_val = agent.update(next_state, done)
...
if __name__ == "__main__":
    ...
    print(f"\n{'=' * 50}")
    print(f"  FINAL RESULTS -- A2C Baseline")
    print(f"{'=' * 50}")
    results = [
        ("A2C",         np.mean(lat[-100:])),
    ...
    fig.suptitle(f"A2C Baseline -- UAV Task Offloading\n"
                 f"{N_DEVICES} Devices, {N_EPISODES} Episodes",
                 fontsize=14, fontweight='bold')

    axes[0, 0].plot(rew, alpha=0.15, color='royalblue')
    axes[0, 0].plot(x_range, moving_avg(rew, window),
                    color='royalblue', linewidth=2.5, label='A2C')
    ...
    axes[0, 1].plot(lat, alpha=0.15, color='royalblue')
    ...
    axes[1, 0].plot(losses, alpha=0.15, color='royalblue')
    ...
    plt.tight_layout()
    plt.savefig('outputs/5_a2c_baseline.png', dpi=150, bbox_inches='tight')
    plt.show()
    print("\nPlot saved as outputs/5_a2c_baseline.png")
